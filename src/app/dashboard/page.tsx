...
function generateKey(): string {
  ...
  return 'agp_' + Array.from(arr).map(b => b.toString(16).padStart(2, '0')).join('')
}
...
export default function Dashboard() {
  ...
  const [newKeyPreview, setNewKeyPreview] = useState<string | null>(null)

  useEffect(() => {
    const stored = localStorage.getItem('agentpay_api_key')
  ...
  function handleGenerateKey() {
    if (apiKey) {
      ...
      setNewKeyPreview(next)
      setShowKeyWarning(true)
    } else {
      const key = generateKey()
      localStorage.setItem('agentpay_api_key', key)
      setApiKey(key)
      setKeyVisible(true)
    }
  }
  ...
  function confirmRegenerate() {
    if (!newKeyPreview) return
    localStorage.setItem('agentpay_api_key', newKeyPreview)
    setApiKey(newKeyPreview)
    setKeyVisible(true)
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-6">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-white/40 mt-0.5">Use this key to authenticate SDK calls with <code className="bg-white/10 px-1 rounded">x-api-key</code></p>
            </div>
            <button
          ...
          {apiKey ? (
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-black/30 border border-white/10 rounded-lg px-3 py-2 font-mono text-sm text-white/80 overflow-hidden">
                {keyVisible ? apiKey : maskKey(apiKey)}
              </div>
              <button
                onClick={() => setKeyVisible(v => !v)}
                className="text-white/40 hover:text-white/80 px-2 py-2 rounded-lg hover:bg-white/5 transition-colors text-xs"
              ...
              <button
                onClick={copyKey}
                className={`text-xs px-3 py-2 rounded-lg transition-colors ${keyCopied ? 'bg-green-500/20 text-green-400' : 'bg-white/5 hover:bg-white/10 text-white/60'}`}
              >
                {keyCopied ? '✓ Copied' : 'Copy'}
              </button>
            </div>
          ) : (
            <div className="text-center py-6 text-white/30 text-sm border border-dashed border-white/10 rounded-lg">
              No API key yet — click <strong className="text-white/50">Generate Key</strong> to get started
            </div>
          )}

          {apiKey && (
            <p className="text-xs text-white/30 mt-2">
              Example: <code className="bg-white/5 px-1 rounded">const pay = new AgentPay({'{ apiKey: "'}{ maskKey(apiKey)}{'"}'})</code>
            </p>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-[#13131f] border border-white/10 rounded-2xl p-6 w-full max-w-md">
            ...
            <p className="text-sm text-white/60 mb-4">
              Your current key will stop working immediately. Any SDK integrations using the old key will fail until updated.
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => { setShowKeyWarning(false); setNewKeyPreview(null) }}
                className="flex-1 bg-white/5 hover:bg-white/10 text-white text-sm px-4 py-2 rounded-lg transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-white text-sm px-4 py-2 rounded-lg transition-colors"
              >
                Yes, Regenerate
              </button>
            </div>
